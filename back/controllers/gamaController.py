...
def _get_saved_latest_analises_from_db(user_id: str, categoria: str) -> dict:
    """
    ...
    try:
        ...
        for r in rows:
            analises.append({
                ...
                "evidencias": r[3] or [],
                "plano_de_acao": r[4] or [],
                "riscos_associados": r[5] or [],
...
def _build_input_text_from_analises(payload: dict) -> str:
    ...
    categoria = payload.get("categoria", "produto").capitalize()
    ...
    for a in analises:
        ...
        if a.get("evidencias"):
            lines.append("\n## Evidências & Dados")
            for item in a["evidencias"][:5]:
                lines.append(f"* {item}")

        if a.get("riscos_associados"):
            lines.append("\n## Riscos Identificados")
            for item in a["riscos_associados"][:5]:
                lines.append(f"* {item}")

        if a.get("plano_de_acao"):
            lines.append("\n## Plano de Ação Recomendado")
            for item in a["plano_de_acao"][:5]:
                lines.append(f"* {item}")
