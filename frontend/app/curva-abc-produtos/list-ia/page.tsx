...
function chipCountSx(kind: 'evidence' | 'risk' | 'action') {
  if (kind === 'evidence') {
    return {
      ...
      fontWeight: 600,
      '&:hover': { bgcolor: alpha(STATUS_COLORS.info, 0.2) },
    };
  }
  if (kind === 'risk') {
    return {
      ...
      fontWeight: 600,
      '&:hover': { bgcolor: alpha(STATUS_COLORS.error, 0.2) },
    };
  }
  if (kind === 'action') {
    return {
      ...
      fontWeight: 600,
      '&:hover': { bgcolor: alpha(STATUS_COLORS.success, 0.2) },
    };
  }
  return {
...
export default function ListIAProdutosPage() {
  ...
  const loadAnalises = async () => {
    ...
    const res = await services(`/toon/insights/saved/latest?${params.toString()}`, { method: 'GET' });
    ...
    const payload: ApiResponse | null =
      res.data && (Array.isArray(res.data.analises) || Array.isArray(res.data.analises?.analises))
  ...
  const totalEvidencias = useMemo(() => {
    return analises.reduce((acc, a) => acc + (a.evidencias?.length || 0), 0);
  }, [analises]);

  const totalRiscos = useMemo(() => {
    return analises.reduce((acc, a) => acc + (a.riscos_associados?.length || 0), 0);
  }, [analises]);

  const totalAcoes = useMemo(() => {
    return analises.reduce((acc, a) => acc + (a.plano_de_acao?.length || 0), 0);
  }, [analises]);
          ...
          >
            ...
            <CardContent sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography sx={{ color: GRAY_MAIN, mb: 4, maxWidth: 360, mx: 'auto' }}>
                Para visualizar as análises, primeiro selecione um cliente no menu lateral.
              </Typography>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              {loading ? (
                ...
                >
                  ...
                  <Button
                    variant="contained"
                    onClick={() => router.push('/curva-abc-produtos/produtos')}
                    sx={{
                      textTransform: 'none',
                      fontWeight: 600,
                      borderRadius: 2,
                      ...
                      <Fade in timeout={300 + index * 50}>
                        ...
                        >
                          ...
                          <Stack spacing={2.5}>
                            ...
                            >
                              {analise.situacao_identificada}
                            </Typography>

                            <Divider sx={{ borderColor: BORDER_LIGHT }} />

                            {/* Contadores com badges */}
                ...
                <Box>
                  ...
                  <Stack direction="row" spacing={2} alignItems="center" sx={{ mt: 0.5 }}>
                    ...
                    <Chip
                      ...
                      size="small"
                      sx={{ bgcolor: alpha(GRAY_MAIN, 0.05), color: GRAY_MAIN, fontFamily: 'monospace' }}
                    />
                  </Stack>
                </Box>
              </Stack>
            ...
            <Fade in timeout={500}>
              <Stack spacing={4}>
                ...
                >
                  ...
                  <Typography variant="body1" sx={{ color: GRAY_MAIN, lineHeight: 1.6 }}>
                    {selected.situacao_identificada}
                  </Typography>
                </Paper>
                    ...
                    >
                      ...
                      <Stack spacing={1.5}>
                        {selected.riscos_associados?.map((risco, idx) => (
                          ...
                          >
                            <Typography variant="body2" sx={{ color: GRAY_MAIN, lineHeight: 1.5 }}>
                              • {risco}
                            </Typography>
                          </Paper>
                        ))}
