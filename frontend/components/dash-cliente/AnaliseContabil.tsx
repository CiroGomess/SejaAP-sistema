...
export default function AnaliseContabil({ data }: AnaliseContabilProps) {
    const receitas = Number(data?.receitas || 0);
    const despesas = Number(data?.despesas || 0);
    const resultado = Number(data?.resultadoLiquido || 0);
                    ...
                    <Grid size={{ xs: 12 }}>
                        ...
                        >
                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                                ...
                                <Chip
                                    icon={isPositivo ? <TrendingUpIcon /> : <TrendingDownIcon />}
                                    label={`${margem.toFixed(1)}%`}
                                    sx={{
                                        bgcolor: alpha(isPositivo ? '#10B981' : '#EF4444', 0.1),
                                        color: isPositivo ? '#10B981' : '#EF4444',
                                        fontWeight: 700,
                                    }}
                                />
